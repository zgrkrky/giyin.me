...
import { uploadGarment } from '../services/geminiService';
...
const WardrobePanel: React.FC<WardrobePanelProps> = ({ onGarmentSelect, activeGarmentIds, isLoading, wardrobe }) => {
  ...
  try {
    setError(null);
    console.log('[uploadGarment] about to upload:', file.name);

    // 1) Dosyayı backend'e yükle → signed URL al
    const uploadedUrl = await uploadGarment(file);

    // 2) Gardırop item bilgisi oluştur ve parent'a ilet
    const customGarmentInfo: WardrobeItem = {
      id: `custom-${Date.now()}`,
      name: file.name,
      url: uploadedUrl, // artık kalıcı (signed) URL
    };

    onGarmentSelect(file, customGarmentInfo);
  } catch (err: any) {
    console.error('Kıyafet yükleme hatası:', err);
    setError(err?.message || 'Kıyafet yüklenirken bir hata oluştu.');
  } finally {
  ...
  return (
    <div className="pt-6 border-t border-gray-400/50">
        <h2 className="text-xl font-serif tracking-wider text-gray-800 mb-3">Gardırop</h2>
        <div className="grid grid-cols-3 gap-3">
            ...
            <label htmlFor="custom-garment-upload" className={`relative aspect-square border-2 border-dashed rounded-lg flex flex-col items-center justify-center text-gray-500 transition-colors ${isLoading ? 'cursor-not-allowed bg-gray-100' : 'hover:border-gray-400 hover:text-gray-600 cursor-pointer'}`}>
                ...
                <span className="text-xs text-center">Yükle</span>
        ...
        {wardrobe.length === 0 && (
             <p className="text-center text-sm text-gray-500 mt-4">Yüklediğin Kıyafetler Burada Görünecek</p>
        )}
